...
vector<moves> cannon_shift(int **board,vector<pair<pair<int,int>,int> > cannon,int n,int m){
	...
	for(int i=0;i<cannon.size();i++){
		...
		if(head==1 && x+1<m){
			...
			if(board[x][y+1]==0){
				vm.a='S';vm.b=(x);vm.c=(y-2);vm.d='M';vm.e=(x);vm.f=(y+1);ans.push_back(vm);
				//cerr<<vm.a<<vm.b<<vm.c<<vm.d<<vm.e<<vm.f<<endl;
		    }
		}
		else if(head==-2 && y-1>=0){
			if(board[x][y-1]==0){
...
int eval(bool townhall_kill, int **board,int n,int m, pair<int,int> *Soldier,pair<int,int> *EnemySoldier, pair<int,int> *Townhall, pair<int,int> *EnemyTownhall, vector<pair<pair<int,int>,int> > cannon, vector<pair<pair<int,int>,int> > Enemycannon){
	...
	int weight_soldier = 30;
	int weight_soldier_fire = 20;
	int weight_cannon = 25;
	int weight_townhall = 100;
	int weight_townhall_fire = 50;
	if(EnemyTownhall[0].first==3 && Townhall[0].first==4){
		weight_townhall = 70;
	}
...
pair<moves, int> min_value_enemy(int player_num,bool townhall_kill,int **board, int n, int m, int alpha, int beta, moves ms, int min_val, pair<int,int> *Soldier, pair<int,int> *EnemySoldier, pair<int,int> *Townhall, pair<int,int> *EnemyTownhall, vector<pair<pair<int,int>,int> > cannon, vector<pair<pair<int,int>,int> > Enemycannon){
	...
	for(int i=0;i<v.size();i++){
		...
		if(min_child.second<=alpha){
			//cerr<<min_val<<"................"<<min_child.first.a<<min_child.first.b<<min_child.first.c<<min_child.first.d<<min_child.first.e<<min_child.first.f<<endl;
			return min_child;
		}
